            ...
            {loading ? <Loader /> :
                <Background>
                    <div className="relative z-50 flex h-screen">
                        <aside className="flex flex-col gap-2 w-1/4 text-white">
                            <div className="flex items-center gap-1 pt-4 px-4">
                                <img src="/quickchat.webp" className="w-10 h-10" alt="Logo Quick Chat" />
                                <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-blue-600 cursor-pointer">Quick Chat</h2>
                            </div>
                            <div className="sticky top-0 bg-transparent backdrop-blur-xl px-5">
                            ...
                            <ul className="max-h-[calc(100vh-10rem)] overflow-y-auto">
                                {results.length > 0 ? (
                                    results.map((chat) => (
                                        <motion.div
                                            key={chat.id}
                                            className="flex gap-2 px-4 py-3 hover:bg-gray-800 cursor-pointer font-normal"
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col justify-center">
                                                <p className="text-base/tight font-normal">{chat.name}</p>
                                                <div className="flex gap-1">
                                                    <span className="flex items-center justify-center">
                                                        <MessageStatus status="delivered" isSeen={false} />
                                                    </span>
                                                    <div className="flex items-end">
                                                        <p className="pt-1 text-sm/tight text-gray-500 font-semibold">{chat.message}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </motion.div>
                                    ...
                                    <li className="mb-3 p-2 ml-3 font-semibold">No se encontraron resultados.</li>
                                )}
                            </ul>
                        </aside>
